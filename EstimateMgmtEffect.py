...
arcpy.management.MakeFeatureLayer(AOI_fc,\
    ...
    # Management polygon
MgmtArea_fc = arcpy.GetParameterAsText(4)

#MgmtArea_fc = r'C:\Users\eric.chabot\Documents\NGPJV_spatial_tool\NGPJVSpatialTool\NGPJVSpatialTool.gdb\Mgmt_Area' # For testing

MgmtArea = "MgmtArea"
arcpy.management.MakeFeatureLayer(MgmtArea_fc,\
                                      MgmtArea, \
                                      '', None,\
                                      "")

# Create a raster that has the value of -1 in the area of interest and 1 in the management area, will be used later
MgmtIdentity = ModelLayersGDB + '\\MgmtIdentity'
IdentityRas = ModelLayersGDB + '\\IdentityRas'

arcpy.analysis.Identity(AOI, \
                        MgmtArea, \
                        MgmtIdentity, \
                        "ONLY_FID", None, "NO_RELATIONSHIPS")

RasterValueField = "FID_"+arcpy.Describe(MgmtArea_fc).basename
...
with arcpy.EnvManager(snapRaster=ModelLayersGDB + '\Lat', outputCoordinateSystem = arcpy.Raster(ModelLayersGDB + '\Lat').spatialReference):
    arcpy.conversion.PolygonToRaster(MgmtIdentity,\
                                     RasterValueField, \
                                     IdentityRas,\
                                     "CELL_CENTER", "NONE",\
                                     ModelLayersGDB + '\Lat', "BUILD")

############
    # Management levels to measure as an effect on future abundance
    
#AGFCpct = 0 # For testing
AGFCpct = arcpy.GetParameterAsText(5)
#PGFCpct = 0 # For testing
PGFCpct = arcpy.GetParameterAsText(6)
#SHRBpct = 200 # For testing
SHRBpct = arcpy.GetParameterAsText(7)
#TREEpct = 5000 # For testing
TREEpct = arcpy.GetParameterAsText(8)

GZpct = arcpy.GetParameterAsText(9)
#EApct = 0 # For testing
EApct = arcpy.GetParameterAsText(10)

    # Number of years to predict beyond the latest data
#PredYrs = 5 # For testing
PredYrs = arcpy.GetParameterAsText(11)
PredictionYears = int(PredYrs)


    # Confidence Interval?
CIsize = arcpy.GetParameterAsText(12)
#CIsize = 95
per1 = int(CIsize)
per2 = 100 - per1
...
arcpy.SetProgressor("step", "Generating Management Rasters...",
    ...
    # Create the rasters themselves
out_raster_agfc = (Raster(ModelLayersGDB + '\AGFC_median')*(int(AGFCpct)/100))
out_raster_pgfc = (Raster(ModelLayersGDB + '\PGFC_median')*(int(PGFCpct)/100))
out_raster_shrb = (Raster(ModelLayersGDB + '\SHRB_median')*(int(SHRBpct)/100))
out_raster_tree = (Raster(ModelLayersGDB + '\TREE_median')*(int(TREEpct)/100))

# Create a scale raster that's the sum of the veg cover values
RAP_Scale_Ras = out_raster_tree + out_raster_shrb + out_raster_pgfc + out_raster_agfc
RAP_Sum = Raster(ModelLayersGDB + '\AGFC_median') + Raster(ModelLayersGDB + '\PGFC_median') + Raster(ModelLayersGDB + '\SHRB_median') +Raster(ModelLayersGDB + '\TREE_median')

scaled_tree = RAP_Sum*out_raster_tree / RAP_Scale_Ras
out_ras = Con(IdentityRas, ModelLayersGDB + '\TREE_median', scaled_tree, "VALUE = -1")
out_ras.save(ModelLayersGDB + '\\TREE_1k_mgmt')

scaled_pgfc = RAP_Sum*out_raster_pgfc / RAP_Scale_Ras
out_ras = Con(IdentityRas, ModelLayersGDB + '\PGFC_median', scaled_pgfc, "VALUE = -1")
out_ras.save(ModelLayersGDB + '\\PGFC_1k_mgmt')

scaled_agfc = RAP_Sum*out_raster_agfc / RAP_Scale_Ras
out_ras = Con(IdentityRas, ModelLayersGDB + '\AGFC_median', scaled_agfc, "VALUE = -1")
out_ras.save(ModelLayersGDB + '\\AGFC_1k_mgmt')

scaled_shrb = RAP_Sum*out_raster_shrb / RAP_Scale_Ras
out_ras = Con(IdentityRas, ModelLayersGDB + '\SHRB_median', scaled_shrb, "VALUE = -1")
out_ras.save(ModelLayersGDB + '\\SHRB_1k_mgmt')

out_raster = (Raster(ModelLayersGDB + '\\Trt_EA')+int(EApct)); \
             out_raster2 = Con(IdentityRas, ModelLayersGDB + '\\Trt_EA', out_raster, "VALUE = -1");\
             arcpy.SetProgressorPosition();\
             out_raster3 = Con(out_raster2,100, out_raster2,"VALUE > 100");\
             arcpy.SetProgressorPosition();\
             out_raster3.save(ModelLayersGDB + '\\Trt_EA_mgmt');\
             arcpy.SetProgressorPosition()

out_raster = (Raster(ModelLayersGDB + '\\Trt_GZ')+int(GZpct)); \
             out_raster2 = Con(IdentityRas, ModelLayersGDB + '\\Trt_GZ', out_raster, "VALUE = -1");\
             arcpy.SetProgressorPosition();\
             out_raster3 = Con(out_raster2,100, out_raster2,"VALUE > 100");\
             arcpy.SetProgressorPosition();\
             out_raster3.save(ModelLayersGDB + '\\Trt_GZ_mgmt');\
             arcpy.SetProgressorPosition()
...
arcpy.SetParameter(13,ResultsGDB+"\\Abundance_"+str(RefYear)+"_"+Spp)

arcpy.SetParameter(14,ResultsGDB+"\\Abundance_"+str(max(all_years))+"_"+Spp)

arcpy.SetParameter(15,ResultsGDB+"\\Abundance_"+str(max(all_years))+"_mgmt_"+Spp)

arcpy.SetParameter(16,ResultsGDB+"\\Trend_"+Spp)

arcpy.SetParameter(17,ResultsGDB+"\\Trend_mgmt_"+Spp)

arcpy.SetParameter(18,ResultsGDB+"\\StatisticallyStrong_"+Spp)
...
arcpy.SetParameterSymbology(15, outsymAbun)

arcpy.SetParameterSymbology(16, outsymTrend)
arcpy.SetParameterSymbology(17, outsymTrend)
arcpy.SetParameterSymbology(18, outsymSS)
